...
import com.novatorem.ttvlc.tasks.HandlerUserLoginTask;
...
public class LoginActivity extends UsageTrackingAppCompatActivity {
	...
	//private final String LOGIN_URL = "https://api.twitch.tv/kraken/oauth2/authorize?response_type=token&client_id=" + Service.getApplicationClientID() + "&redirect_uri=https%3A%2F%novac.dev%2Foauth_authorizing&scope=user_read+chat_login+user_follows_edit+user_subscriptions";
	private final String LOGIN_URL = "https://id.twitch.tv/oauth2/authorize?client_id=" + Service.getApplicationClientID() + "&redirect_uri=https://novac.dev/x/twitch/&response_type=code&scope=user_read+user_follows_edit+user_subscriptions";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContinueFABContainer.setOnClickListener(v -> {
			...
			if (TwitchToVLCApplication.isCrawlerUpdate)
	...
	public void handleLoginSuccess() {
		...
		// if the user later removes TTV Authentication from Twitch and we need the user to sign in again
	...
	public void handleLoginFailure() {
		Log.w("handleLoginFailure", "Log in failed.");
	}

	public void handleNoInternet() {
		Log.w("TTV-Debug", "No Internet");
	...
	private void handleUserCancel() {
		Log.w("TTV-Debug", "User cancelled");
		mSnackbar
	...
	private void showLoginView() {
		Log.w("TTV-Debug", "Showing login view");
	...
	private AnimationSet hideLoginView() {
		Log.w("TTV-Debug", "Hiding login view");
		showFABAnimation();
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
